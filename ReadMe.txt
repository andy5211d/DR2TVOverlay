...
The script uses the data provided by DR's DR2Video software and its associated text or XML files.  Once running the script
...
a skills circuit!  The information display needs to use a Mono Type font (DejaVu Sans Mono Book) which needs to be loaded on
the PC, else the awards alignment will be wrong and some data may be outside the video display area.  This script will not 
function correctly without the OBS Source's generated by the JASON file which needs to be imported by the user into OBS.
