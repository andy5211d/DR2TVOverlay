...
**                         between modes, likely to change though!
...
**    V3.5.1   2024-12-04  First change to make the main banner size relate to the number of judges.   *** DOES NOT DO ANYTHING DIFFERENT YET ***
**                         Without a complete re-design of the code it looks like its only practical to have two displays, one for 7 and above judge numbers and one for individual events with 5 or 3 judges. It may
**                         be possible to make the individual 5 judge banner work for 5 judge synchro as well.   Not sure!
**    V3.5.2   2026-04-01  Change to show Team rather the Diver in the event banner for Synchro event.
**
...
local function single_update(v)
    ...
    --  Event Complete?
        ...
        if split_string2[16] == "1" or split_string2[16] == "3" or split_string2[16] == "5" or split_string2[16] == "7½" or split_string2[16] == "7.5" or split_string2[16] == "10" then 
...
local function simultaneous_update(v)
    ...
    if synchro then 
        ...
        displayText = (" ")
        if eventB then
            local source = obs.obs_get_source_by_name("EventData_B") -- Display data for B (actually nothing)
            if source ~= nil then
            local settings = obs.obs_data_create()
            ...
            local source = obs.obs_get_source_by_name("EventData_A") -- Display data for A (actually nothing)
            if source ~= nil then
            local settings = obs.obs_data_create()
                obs.obs_data_set_string(settings, "text", displayText)
                obs.obs_source_update(source, settings)
    ...
    local source = obs.obs_get_source_by_name("EventData_A") -- enable EventData_A Source group for display of dive/awards on the left
    if source ~= nil then
        obs.obs_source_set_enabled(source, true)
        if debug then
            obs.script_log(obs.LOG_INFO, string.format("Enable_sourcedata at start of simultaneous_update(): " .. "EventData_A "))
        end
        obs.obs_source_release(source)
    end    
    ...
    local source = obs.obs_get_source_by_name("EventData_B") -- enable Event B Source group for display of dive/awards on the right
    if source ~= nil then
        obs.obs_source_set_enabled(source, true)
        if debug then
            obs.script_log(obs.LOG_INFO, string.format("Enable_sourcedata at start of simultaneous_update(): " .. "EventData_B"))
        end
        obs.obs_source_release(source)            
    end
    ...
    if split_string2[17] ~= (" ") then -- if award in J1 position then display judge awards  -- is there a better way of doing this now we have UDP data?
        ...
        -- Event Complete ??
--[[    this makes no sense for simultaneous events!!  
        if split_string2[8] == split_string2[64] and split_string2[6] == split_string2[63] then
            eventComplete = true
            ...
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("An Event Complete!"))
            end
        else
            eventComplete = false
            local source = obs.obs_get_source_by_name("Event_Complete") -- disable blue status rectangle in 'Status' source dock
        ...
        -- generate the first line of the display.   Event, then alternativy the round number or the diver number.
        display1a = (" " .. split_string2[60] .. "  ")                                  -- first line: Event info
        display1b = (" Dvr " .. split_string2[8] .. "/" .. split_string2[64] .. " ")    -- first line: diver number of total divers option
        display1c = (" Rnd " .. split_string2[6] .. "/" .. split_string2[63] .. " ")    -- first line: round number of total rounds option
    
        -- insert display's into lineOne
        lineOne = string_insert(lineOne, display1a, 0)        -- insert into line One
        lineOne = string_insert(lineOne, display1b, 33)       -- insert into line One

        -- generate dive position
        if     split_string2[14] == "A" then position = (", straight")
        elseif split_string2[14] == "B" then position = (", piked")
        elseif split_string2[14] == "C" then position = (", tucked")
        elseif split_string2[14] == "D" then position = (", free position")
    ...
    -- however this does not swap A & B around under the command of Hotkey F10 yet!
    if eventB then
        if togglevar2 then
            eventText = ("EventData_B")
        else 
            eventText = ("EventData_A")
        end
        local source = obs.obs_get_source_by_name(eventText) -- Display data for B, until flipped
        if source ~= nil then
        local settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", displayText)
            obs.obs_source_update(source, settings)
        ...
        local source = obs.obs_get_source_by_name(eventText) -- Display data for A, until flipped
        if source ~= nil then
        local settings = obs.obs_data_create()
            obs.obs_data_set_string(settings, "text", displayText)
            obs.obs_source_update(source, settings)
...
function toggle_event_position(pressed)  -- F10 Hotkey to toggle Event overlay or Event A or B positions
    ...
    if simultaneousEvents then --F10 for simultaneous events to swap event A and B positions
        ...
        redisplay_overlays(true)  -- in sumultaneous mode clears left display
        toggle_event_a_or_b(true)  -- in sumultaneous mode clears right display       
--[[   an attempt to redisplay event in correct location after F10 action.  Does not work, need to work on the logic!!
        if resultK[#resultK] ~= nil then  -- not empty so update display
            if resultK[2] == 'b' and togglevar2 == false then
                redisplay_overlays(true)  -- in sumultaneous mode clears left display
                toggle_event_a_or_b(true)  -- in sumultaneous mode clears right display
--                simultaneous_update(resultK)  -- update display 
                display_overlays(true) 
            end
            if resultK[2] == 'a' and togglevar2 == false then
                redisplay_overlays(true)  -- in sumultaneous mode clears left display
--                toggle_event_a_or_b(true)  -- in sumultaneous mode clears right display
                simultaneous_update(resultK)  -- update display 
                display_overlays(true)
    ...
    else  -- F10 for single event text overlay position
...
function toggle_simultaneous_events(pressed)  -- F12 Hotkey to toggle Simultaneous Events                                         
    ...
    eventComplete = false  -- remove event complete flag in status area
    if simultaneousEvents then  -- change to single event operation
        local source = obs.obs_get_source_by_name("F12 - Simultaneous Events") -- disable Simultaneous Event status banner
        if source ~= nil then
            obs.obs_source_set_enabled(source, false)
            obs.obs_source_release(source)            
        end
        local source = obs.obs_get_source_by_name("F12 - Single Event") -- enable Single Event status banner
        if source ~= nil then
            obs.obs_source_set_enabled(source, true)
            obs.obs_source_release(source)            
        end
        ...
        local source = obs.obs_get_source_by_name("F8_Function_Code") -- enable F8 label
        if source ~= nil then
            obs.obs_source_set_enabled(source, true)
            obs.obs_source_release(source)            
        end
        ...
        local source = obs.obs_get_source_by_name("AutoHide") -- disable F8 'auto_hide' description as not used in Sumultaneous events mode
        if source ~= nil then
            obs.obs_source_set_enabled(source, false)
            obs.obs_source_release(source)            
        end
        local source = obs.obs_get_source_by_name("F8_Function_Code") -- disable 'F8_Function_Code' label as not used in Sumultaneous events mode
        if source ~= nil then
            obs.obs_source_set_enabled(source, false)
            obs.obs_source_release(source)            
        end
        local source = obs.obs_get_source_by_name("F3_ClearEventA") -- enable 'F3_ClearleftEvent' text 
        if source ~= nil then
            obs.obs_source_set_enabled(source, true)
            obs.obs_source_release(source)            
        end
        local source = obs.obs_get_source_by_name("F6_Function_Background_False") -- disable 'F6_Function_Background_False'.  Ensure correct colour!
        if source ~= nil then
            obs.obs_source_set_enabled(source, false)
            obs.obs_source_release(source)            
        end        
        local source = obs.obs_get_source_by_name("F6_Function_Background_True") -- enable 'F6_Function_Background_True'.  Ensure correct colour!
        if source ~= nil then
            obs.obs_source_set_enabled(source, true)
            obs.obs_source_release(source)            
        end          
        local source = obs.obs_get_source_by_name("F6_ClearEventB") -- enable 'F6_ClearRightEvent' text 
        if source ~= nil then
            obs.obs_source_set_enabled(source, true)
            obs.obs_source_release(source)            
        end        
        ...
        local source = obs.obs_get_source_by_name("F12 - Single Event") -- disable single event status banner
        if source ~= nil then
            obs.obs_source_set_enabled(source, false)
            obs.obs_source_release(source)            
        end
        local source = obs.obs_get_source_by_name("F12 - Simultaneous Events") -- enable simultaneous event status banner
        if source ~= nil then
            obs.obs_source_set_enabled(source, true)
            obs.obs_source_release(source)            
        end
...
function remove_TVbanner()  -- this removes the overlay under timer control
    ...
    --[[
    else  -- leftover from auto overlay remove at the end of the event.  Not reliable and when the Recorders display Results or Rankings the banner re-displays!!  What UDP data drives this re-display?
    ...
    repeat  -- first port (58091)
        ...
        if dataClient then
             if currentDataClient ~= dataClient then
                ...
                fileContentsChanged = true    -- not actually a file (from a UDP port!) but keep this logic anyway
                eventComplete = false                
                ...
                processMessage(dataClient, '', '', '')  -- allow for receiving messages from four ports, using only port 58091 here
            else 
...
key_2 = '  "htk_2": [ { "key": "OBS_KEY_F2" } ], '      -- HK to temp display_overlays (most of them)
key_3 = '  "htk_3": [ { "key": "OBS_KEY_F3" } ], '      -- HK to redisplay_overlays or remove left event text
key_4 = '  "htk_4": [ { "key": "OBS_KEY_F4" } ], '      -- HK to do nothing!
key_5 = '  "htk_5": [ { "key": "OBS_KEY_F5" } ], '      -- HK to permanently remove overlays
key_6 = '  "htk_6": [ { "key": "OBS_KEY_F6" } ], '      -- HK to toggle select A or B Event or remove right event text
key_7 = '  "htk_7": [ { "key": "OBS_KEY_F7" } ], '      -- HK to do nothing!
key_8 = '  "htk_8": [ { "key": "OBS_KEY_F8" } ], '      -- HK to disable auto-hide of overlays
key_9 = '  "htk_9": [ { "key": "OBS_KEY_F9" } ], '      -- HK to toggle simultaneous events mode. Not used now as event type changed by UDP data
key_10 = '  "htk_10": [ { "key": "OBS_KEY_F10" } ], '   -- HK to toggle_event_position or HK to toggle Event A or Event B position or to swap positions when in simultaneous mode
key_12 = '  "htk_12": [ { "key": "OBS_KEY_F12" } ] } '  -- HK to toggle_operating mode (single or simultaneous)
                                                      
json_s = key_1 .. key_2 .. key_3 .. key_4 .. key_5 .. key_6 .. key_7 .. key_8 .. key_9 .. key_10 .. key_12
default_hotkeys = {
    ...
    {id='htk_4', des='Not Used ',                                    callback=do_nothing},
    {id='htk_5', des='Permanently Remove All Overlays ',             callback=toggle_display_disable}, 
    {id='htk_6', des='Toggle to Display Event A or Event B ',        callback=toggle_event_a_or_b},  
    {id='htk_7', des='Not used ',                                    callback=do_nothing},
    {id='htk_8', des='Disable Auto-hide of Overlays ',               callback=toggle_disable_of_autohide},
    {id='htk_9', des='Toggle Event Type (Synchro or Individual) ',   callback=toggle_event_type},
    {id='htk_10', des='Toggle Event Overlay Position ',               callback=toggle_event_position},
    {id='htk_12', des='Toggle to/from Simultaneous Event Overlays ',  callback=toggle_simultaneous_events},  
}
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data as a Video Overlay or Overlays</h></center>
             <p>Display diver and scores from DiveRecorder for single individual or synchro diving event or simultaneous individual events.  The approporate OBS Source (.json) file must be imported into OBS for this video overlay to function correctly. You must be connected to the same Class C sub-net as the DR computers. </p><p>Andy - V3.4.2 2024Nov20</p>]]
end
