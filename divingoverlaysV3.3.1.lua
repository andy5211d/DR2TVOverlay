...
function do_nothing(pressed)  --   Hotkey
    if not pressed then
     return
    end
    return
...
function toggle_event_a_or_b(pressed)  -- F6 Hotkey to toggle Event A or Event B and re-start  *** this Function needs to be changed to change positions of A & B overlays for simul events.
    if not pressed then
        return
    end
...
key_6 = '  "htk_6": [ { "key": "OBS_KEY_F6" } ], '      -- HK to toggle select A or B Event
key_7 = '  "htk_7": [ { "key": "OBS_KEY_F7" } ], '      -- HK to do nothing!
key_8 = '  "htk_8": [ { "key": "OBS_KEY_F8" } ], '      -- HK to disable auto-hide of overlays
key_9 = '  "htk_9": [ { "key": "OBS_KEY_F9" } ], '      -- HK to toggle simultaneous events mode. Need different methodology for key modifiers such as Ctrl or Shft
...
key_12 = '  "htk_12": [ { "key": "OBS_KEY_F12" } ] } '  -- HK to toggle_event_type (synchro or individual)
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data as a Video Overlay or Overlays</h></center>
             <p>Display diver and scores from DiveRecorder for single individual or synchro diving event or simultaneous individual events.  The approporate OBS Source (.json) file must be imported into OBS for this video overlay to function correctly. You must be connected to the same Class C sub-net as the DR computers. </p><p>Andy - V3.3.1 2023NOV10</p>]]
end
