...
**    V3.1.1   2023-04-20  Minor bug fixe for F5 hotkey which did not do anything! Re-purpose F3 to be 'Re-display Overlay' hotkey.                     
**    
**  
...
--        local resultK = {} -- empty array where we will store data from the first UDP port data stream
        ...
        --  resultK is generated array with entries from the UDP data packet             
        ...
        if eventB then
            if resultK[1] == "REFEREE" and resultK[2] == "b" then  
                synchro = false   
                if not disableUpdate then -- has F5 been pressed dont update?
...
function init()
    ...
    activeId = activeId + 1
     ...
     -- start the timer id to check changed data.  Poss not needed now using UDP but removal stops something and does not work but don't know why at this time :-)
    id = activeId
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data as a Video Overlay</h></center>
             <p>Display diver and scores from DiveRecorder for individual and synchro diving events.  The approporate OBS Source (.json) file must be imported into OBS for this video overlay to function correctly. You must be connected to the same Class C sub-net as the DR computers. </p><p>Andy - V3.1.1 2023Apr20</p>]]
end
