...
local function update(v)
    ...
    if synchro then --  >>>> *** If a Synchro event then ... *** <<<<<
        ...
        if split_string2[17] ~= (" ") then -- if awards in J1 field then display them
            ...
            obs.obs_source_release(source)
            if split_string2[26] ~= (" ") then -- if award in J10 then 11 synchro judges (Judge role labels are in different positions for 9 judges!)
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data as a Video Overlay</h></center>
             <p>Display diver and scores from DiveRecorder for individual and synchro diving events.  The approporate OBS Source (.json) file must be imported into OBS for this video overlay to function correctly. You must be connected to the same Class C sub-net as the DR computers. </p><p>Andy - V3.0.1 2022Nov02</p>]]
end
