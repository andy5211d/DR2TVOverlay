...
**  matching OBS Source JSON File :- divingoverlays-sourceV4.y.z.json  (for V3.2.2)
**
...
**    V3.2.2   2023-10-24  Re-factored for single_update and simultaneous_update screen update functions (functions may not be called that in the end though).  Ctrl or Shfy F12 sot working.    
**
**  
...
function getPath(str)   -- Sorts out the Windows file path separetor character 
    return str:match("(.*[/\\])")
end
...
local function single_update(v)
    ...
    split_string2 = v
    -- now we are processing a UDP message 

    -- update the OBS Status display (old F9) with current settings.
    if synchro then   -- message processing for status update, further down for dive overlay generation (line 285 or thereabouts)
    ...
    if split_string2[32] == ("") then 
       split_string2[32] = (" ")
       obs.script_log(obs.LOG_INFO, string.format("Nul detected in Rank field (first round?)"))
    end
    eventComplete = false
    ...
    if synchro then --  >>>> *** If a SYNCHRO EVENT then process the data *** <<<<<
        ...
        if split_string2[17] ~= (" ") then -- if awards in J1 field then display them
           ...
           -- scores2 = split_string2[30]  -- no data to put in this source as no awards
            ...
            if split_string2[16] == "1" or split_string2[16] == "3" or split_string2[16] == "5" or split_string2[16] == "7.5" or split_string2[16] == "10" then 
            ...
            display2b = "!"
            display2a = "!"
            lineTwo = string_insert(lineTwo, sourcelineTwo, 0)
        ...
        if split_string2[17] ~= (" ") then -- if award in J1 position then display judge awards  -- is there a better way of doing this now we have UDP data?
            ...
            -- lineTwo = string_insert(lineTwo, display2b, 32) -- insert at the end of lineTwo, first part of lineTwo is the awards, but awards are not inserted into this Text Source!
           ...
           -- scores2 = (" ")  -- no data to put in this source as no awards
            ...
            lineTwo = string_insert(lineTwo, sourcelineTwo, 0) -- Insert dive description at the start of lineTwo
            obs.timer_add( function() remove_TVbanner() end,  dinterval )   -- hide overlay after timer period            
        end
    end
...
-- called when a valid UDP message is detected.  Process DR data in the messages then display.  No display timeout for simul displays???
local function simultaneous_update(v)
    ...
    if synchro then 
        -- trap any Synchro event if it gets this far!
        obs.script_log(obs.LOG_INFO, string.format("***** ERROR *****  Can't have a Synchro event when running simultaneous events!"))
        return
    end  
    ...
    if split_string2[17] ~= (" ") then -- if award in J1 position then display judge awards  -- is there a better way of doing this now we have UDP data?
        ...
        display1b = (split_string2[8] .. "/" .. split_string2[64] .. " ")             -- first line: diver number of total divers
        display1c = ("  R " .. split_string2[6] .. "/" .. split_string2[63] .. " ")   -- first line: round number of total rounds

        -- insert display1's into lineOne)
        lineOne = string_insert(lineOne, display1a, 0)        -- insert into line One
        lineOne = string_insert(lineOne, display1c, 34)       -- insert into line One.
        ...
        lineThree = string_insert(lineThree, display3a, 35)
        ...
        -- insert display1's into lineOne)
        lineOne = string_insert(lineOne, display1a, 0)        -- insert into line One
        lineOne = string_insert(lineOne, display1c, 34)       -- insert into line One.  Alternating with display1b, eventually
    ...
    lineTwo = string_insert(lineTwo, display2b, 5)       -- insert into line Two
    lineTwo = string_insert(lineTwo, display2c, 35)      -- insert into line Two
...
function toggle_event_position(pressed)  -- F12 Hotkey  !! Different actions for single event and simultanous events !!
    if not pressed then
     return
    end
...
function toggle_simultaneous_events(pressed)  -- S Hotkey to toggle Simultaneous Events                                         
    ...
    if simultaneousEvents then  -- change to single event operation
        ...
        if source ~= nil then
            obs.obs_source_set_enabled(source, false)
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("disable_source : " .. "Position1b"))
            end
        end
        obs.obs_source_release(source)          
        ...
        obs.obs_source_release(source)    
        local source = obs.obs_get_source_by_name("EventData_A") -- enable simultaneous Event B source (right)
        if source ~= nil then
            obs.obs_source_set_enabled(source, true)
            obs.obs_source_release(source)            
            if debug then
...
--        local resultK = {} -- empty array where we will store data from the first UDP port data stream
        ...
        if simultaneousEvents then
            if resultK[1] == "REFEREE" then
                if resultK[47] == "True" then synchro = true end   -- set this so as to be able to display an error message in simultaneous_update()
            ...
            elseif resultK[1] == "REFEREE" and resultK[2] == "a" then
                ...
                if not disableUpdate then -- has F5 been pressed? if so dont update overlays?
                    single_update(resultK)  -- prosess the 'REFEREE' message for event A                
                end
            end
        end
    ...
    repeat  -- first port (58091)
        ...
        if dataClient then
             if currentDataClient ~= dataClient then
                ...
                fileContentsChanged = true    -- not actually a file but keep logic anyway
                eventComplete = false                
                ...
                processMessage(dataClient, '', '', '')  -- allow for receiving messages from four ports, but using only port 58091 here
...
key_9 = '  "htk_9": [ { "key": "OBS_KEY_S" } ] }' -- HK to toggle simultaneous events mode. Need different methodology for key modifiers such as Ctrl or Shft
                                                                     -- Will need a re-think so as to be able to use a normal function key to switch between single or simultaneous events
                                                                     -- Function keys F4, F6, F7 & F11 are not used yet!
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data as a Video Overlay or Overlays</h></center>
             <p>Display diver and scores from DiveRecorder for single individual or synchro diving event or simultaneous individual events.  The approporate OBS Source (.json) file must be imported into OBS for this video overlay to function correctly. You must be connected to the same Class C sub-net as the DR computers. </p><p>Andy - V3.2.2 2023OCT24</p>]]
end
