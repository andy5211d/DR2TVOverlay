...
**    V3.3.2   2023-11-23  Added capability to swap overlay positions when in Simultaneous mode using the F10 key.  Added F8 and F6 in Simultaneous mode to clear Event A and B respectively. Fixed Penalty bug.
**    V3.3.3   2024-04-07  Minor clarifications.  F6 labels for the different F12 modes now seperated and show correctly
**    V3.3.4   2024-06-15  Removed redundent F8 display in Simultaneous mode.  
**  
...
function toggle_simultaneous_events(pressed)  -- F12 Hotkey to toggle Simultaneous Events                                         
    ...
    if simultaneousEvents then  -- change to single event operation
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Disable_source : F12 - Simultaneous Events"))
            end
        end
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Enable_source : F12 - Single Event"))
            end
        end
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Enable: F8_Function_Code"))
            end
        end
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)        
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Enable_source : " .. "F6 Description"))
            end
        end
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Disable_source : F8_Function_Code"))
            end
        end
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Enable_source : F8_ClearEventA"))
            end
        end
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Enable_source : F6_ClearRightEvent"))
            end
        end        
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Disable_source : " .. "F12 - Single Event"))
            end
        end
        ...
        if source ~= nil then
            ...
            obs.obs_source_release(source)            
            if debug then
                obs.script_log(obs.LOG_INFO, string.format("Enable_source : " .. "F12 - Simultaneous Events"))
            end
        end
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data as a Video Overlay or Overlays</h></center>
             <p>Display diver and scores from DiveRecorder for single individual or synchro diving event or simultaneous individual events.  The approporate OBS Source (.json) file must be imported into OBS for this video overlay to function correctly. You must be connected to the same Class C sub-net as the DR computers. </p><p>Andy - V3.3.4 2024Jun15</p>]]
end
