...
**    V2.1.1  2022-06-28 Code added for UDP receive experiment.  Does not impact the main workings, yet!  Can't enable UDP on this software and have DR2Video running!
**
**  *** Things of note ***
...
--[[
function get_udp_data()  -- udp experiment
    for i = 0, 2, 1 do
    ...
    if data == nil then 
        ...
        obs.script_log(obs.LOG_INFO, string.format(data))
    end
end       
]]
...
local function checkFile(id)
    ...
    --get_udp_dataClient()  -- udp experiment
...
--[[        
    repeat
        ...
        if dataClient then
            ...
            print('dataClient IP: ' .. status:get_ip() .. ',  dataClient port: ' .. status:get_port())
...
function init()
    ...
    local address = socket.find_first_address("*", portClient)
...
--            local dataClient, addr = client:receive_from(address)

            if data then
                obs.script_log(obs.LOG_INFO, string.format(data, addr:get_ip(), addr:get_port()))
    ...
    -- select default Event Display as Position 1 (hide position 2).  Sometimes dos not start correctly, both displayed, not sure why!
    local source = obs.obs_get_source_by_name("Event 2") -- disable text Source (Event group)
        if source ~= nil then
            obs.obs_source_set_enabled(source, false)
        else 
...
--    obs.script_log(obs.LOG_INFO, string.format("HostingClient at: " .. Address1:get_ip() .. ":" .. Address1:get_port()))
--    our_server2 = assert(socket.create("inet", "dgram", "udp"))
--    obs.script_log(obs.LOG_INFO, string.format("HostingServer at: " .. Address2:get_ip() .. ":" .. Address2:get_port()))
--    our_server3 = assert(socket.create("inet", "dgram", "udp"))
--    obs.script_log(obs.LOG_INFO, string.format("HostingWebUp at: " .. Address3:get_ip() .. ":" .. Address3:get_port()))
--    our_server4 = assert(socket.create("inet", "dgram", "udp"))
--    obs.script_log(obs.LOG_INFO, string.format("HostingAwards at: " .. Address4:get_ip() .. ":" .. Address4:get_port()))
...
--    assert(our_server2:bind(Address2, our_port2))
--    assert(our_server3:bind(Address3, our_port3))
--    assert(our_server4:bind(Address4, our_port4))

--    obs.timer_add(UDPtimer_callback, 250)
--    print('Listening on UDP ports ')
end
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data as a Video Overlay</h></center>
             <p>Display diver and scores from DiveRecorder for individual and synchro diving events.  DR2Video text file & path must be entered for main individual (Dive.txt) and for synchro event (Synchro.txt) or the B file if the other event required.  Trigger file locations (DUpdate.txt & SUpdate.txt) need to be entered to trigger an update of the DR data .txt files.  The approporate OBS Source .json file must be imported into OBS for this video overlay to function correctly.  </p><p>Andy - V2.1.1 2022Jun28</p>]]
end
--[[
