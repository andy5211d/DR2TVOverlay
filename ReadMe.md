...
and some data maybe outside the video overlay area (see Installation notes below).  This script will not function 
...
Most user functions can be controlled by hotkeys, as follows:
    ...
    F9 = Toggle event type, Individual or Synchro (automatic, will be overridden on next Diverecorder update)
...
work with both .gif and .jpg format files.  The user will need to change the default flag file to one relevant to their event.
The default flag is set to the 'British Diving' logo.  Re-name your default flag file to 'default.png' as appropriate.
