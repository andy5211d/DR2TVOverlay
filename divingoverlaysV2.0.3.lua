...
local dinterval  -- dinterval = time to display the TV overlay after update
...
local togglevar = true  -- to aid Hotkey Toggle function
local disableUpdate = false -- as the label says!
local tvBanner_removed = false -- 
local fileContentsChanged = false  --

toggle_event_position_hotkey_id   = obs.OBS_INVALID_HOTKEY_ID
remove_overlays_hotkey_id         = obs.OBS_INVALID_HOTKEY_ID
display_overlays_hotkey_id        = obs.OBS_INVALID_HOTKEY_ID
disableUpdate_overlays_hotkey_id  = obs.OBS_INVALID_HOTKEY_ID
...
local function update(k, v)
    ...
    if not ft then
        ...
        obs.script_log(obs.LOG_INFO, string.format("Changed flag file to " .. flag_file))
    else
        ft:close()
    end
...
--        obs.script_log(obs.LOG_INFO, string.format("display2b Length:" .. string.len(display2b) .. " =" .. display2b))
    ...
    obs.script_log(obs.LOG_INFO, string.format("end update(k, v)"))  
end -- update(k, v)
...
local function checkFile(id)
    ...
    -- if disable_Update Hotkey pressed then ignore file update
    ...
    local ft, err = io.open(textFileDI, "rb") --try to open the Update text file, if it exists then its an individual event, process the contents and update TV overlays
    if ft then
        ft:close()
    ...
    local fs, err = io.open(textFileDS, "rb") --try to open the Update text file, if it exists then its a synchro event, process the contents and update TV overlays
...
-- String insert function.  Keeps original string length; well almost! First position is 0, not 1 as per usual with Lua.  So use 0 for the pos vairable if 
...
function disableUpdate_overlays(pressed)  --  Hotkey to disable overlays permenately 
    if not pressed then
     return
    end
    if disableUpdate then
...
function display_overlays(pressed)  -- HotKey to show the two overlays; but not the Synchro Judge Labels
    if not pressed then
     return
    end
    -- tvBanner_remove()
...
function remove_TVbanner()
    ...
    obs.script_log(obs.LOG_INFO, string.format("end remove_TVBanner()"))  
end --remove_TVbanner()
...
function tvBanner_remove()
    ...
    obs.script_log(obs.LOG_INFO, string.format("end tvBanner_remove()"))  
end  -- tvBanner_remove()
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data</h></center>
             <p>Display diver and scores from DiveRecorder for individual and synchro diving events.  DR2Video text file & path must be entered for individual events (Dive.txt) and for synchro events (Synchro.txt).  Trigger file locations (DUpdate.txt & SUpdate.txt) need to be entered to trigger an update of the DR data .txt files.  The approporate OBS Source .json file must be imported into OBS for this video overlay to function correctly.  </p><p>Andy - V2.0.3 2022May30</p>]]
end
