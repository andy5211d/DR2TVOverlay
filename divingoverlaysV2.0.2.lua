...
function remove_TVbanner()
    -- remove the TV Banner from the screen if not hideDisable and after a user configurable period of time.  If last dive then remove banner after time 
    -- period anyway even if hideDisable set.
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data</h></center>
             <p>Display diver and scores from DiveRecorder for individual and synchro diving events.  DR2Video text file & path must be entered for individual events (Dive.txt) and for synchro events (Synchro.txt).  Trigger file locations (DUpdate.txt & SUpdate.txt) need to be entered to trigger an update of the DR data .txt files.  The approporate OBS Source .json file must be imported into OBS for this video overlay to function correctly.  </p><p>Andy - V2.0.2 2022May08</p>]]
end
