...
**  matching Source JSON - divingoverlays-sourceVx.y.z.json
**
...
**    V2.0.2a 2022-05-10  Modified for perm display of overlays for TV company graphics overlay management.
**    V2.0.2b 2022-05-15  Modified to prevent error when update file has no flag file name. Error seems to be triggered by a networked PC actioning a Scoreboard clear command but not when
              the DR server PC sends this command (fix not yet working!).
**
**  *** Things of note ***
...
local event = "Event 1"  -- default location for Event overlay
...
team1_score                = 0
toggle_event_position_hotkey_id   = obs.OBS_INVALID_HOTKEY_ID
...
local function update(k, v)
    ...
    if not ft then
        ...
        flag_file = string.insert(flag_file, "Default.png", index)  -- inset default logo if required flag code not found (string.insert starts at 0, this will produce a sting.insert length error but acceptable as not for formated display)
        obs.script_log(obs.LOG_INFO, string.format("Changed Flag File = " .. flag_file))
    else
        ft:close()
    end
...
function toggle_event_position(pressed)  -- Hotkey to toggle Event overlay position
...
function remove_overlays(pressed)  -- Hotkey to hide all the overlays
...
function clear_overlays(pressed)  -- HotKey to show the two overlays; but not the Synchro Judge Labels  (need to change function name!)
    if not pressed then
     return
    end
    -- tvBanner_remove()
...
function script_load(settings) 
	toggle_event_position = obs.obs_hotkey_register_frontend("toggle_event_position.trigger", "Toggle Event Overlay Position", toggle_event_position) -- Hotkey for Pause
	...
	remove_overlays = obs.obs_hotkey_register_frontend("remove_overlays.trigger", "Remove DR2TVOverlay", remove_overlays)  -- Hotkey for Pause
    ...
    clear_overlays = obs.obs_hotkey_register_frontend("clear_overlays.trigger", "Clear DR2TVOverlays", clear_overlays)  -- Hotkey for Clear
...
function script_description()
    return [[<center><h2>Display DiveRecorder Data</h></center>
             <p>Display diver and scores from DiveRecorder for individual and synchro diving events.  DR2Video text file & path must be entered for individual events (Dive.txt) and for synchro events (Synchro.txt).  Trigger file locations (DUpdate.txt & SUpdate.txt) need to be entered to trigger an update of the DR data .txt files.  The approporate OBS Source .json file must be imported into OBS for this video overlay to function correctly.  </p><p>Andy - V2.0.2a 2022May10</p>]]
end
